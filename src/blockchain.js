...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const now = +new Date()
      const diff = (now - timestamp) / (5 * 60 * 1000)
      if (diff > 5) {
        return reject(new Error('timeout!'))
      }
      try {
        if (!bitcoinMessage.verify(message, address, signature)) {
          return reject(new Error('verify error!'))
        }
      } catch (error) {
        reject(error)
      }
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      resolve(this.chain.find(e => e.hash === hash))
    });
  }
  ...
  getStarsByWalletAddress(address) {
    return new Promise((resolve, reject) => {
      Promise.all(this.chain.map(e => {
        return e.getBData()
      })).then(chain => {
        const stars = chain.filter(e => e.address === address).map(e => ({ owner: e.address, star: e.star }))
